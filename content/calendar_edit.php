...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // คำนวณ engage_date (post_date + 14 วัน)
    $engage_date = NULL;
    if (!empty($post_date)) {
        $engage_date = date('Y-m-d', strtotime($post_date . ' +14 days'));
    }
    
    // อัปเดตข้อมูล
    $sql = "UPDATE calendar_events 
            SET job_title = ?, category = ?, assignee = ?, assignee_username = ?, 
                brief_date = ?, post_date = ?, post_time = ?, engage_date = ?, description = ?, 
                platform = ?, status = ?, updated_at = NOW()
            WHERE id = ?";
    ...
    if ($stmt = mysqli_prepare($conn, $sql)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            try {
                ...
                if (!empty($engage_date)) {
                    $message .= "⏰ วันทำ Engage: " . date('d/m/Y', strtotime($engage_date)) . "\n";
                }
                
                $message .= "👤 แก้ไขโดย: $user_name\n";
                $message .= "\n🔗 ดูรายละเอียด: https://www.weedjai.com/content/calendar_detail.php?id=$event_id";
                
                $pumble->sendToRoles($message, ['admin']);
                
            } catch (Exception $e) {
                error_log("Pumble notification error: " . $e->getMessage());
            }
            
            $success = "แก้ไขงานเรียบร้อยแล้ว!";
            
            // Redirect หลัง 2 วินาที
            header("refresh:2;url=calendar_detail.php?id=$event_id");
        } else {
...
// แปลง platform เป็น array
$selected_platforms = !empty($event['platform']) ? explode(',', $event['platform']) : array();
?>

<!DOCTYPE html>
<html>
<head>
    <title>แก้ไขงานในปฏิทิน</title>
    <meta charset="utf-8" />
...
<div class="container">
    ...
    <div class="info-box">
        <strong>💡 คำแนะนำ:</strong> วันทำ Engage จะถูกคำนวณอัตโนมัติเป็น 14 วันหลังจากวันที่โพสต์
    </div>
    
    <form method="POST" action="">
        <div class="form-group">
            <label>ชื่องาน / หัวข้อ <span class="required">*</span></label>
            <input type="text" name="job_title" value="<?php echo htmlspecialchars($event['job_title']); ?>" required>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="category" required>
                ...
                <option value="Product Knowledge" <?php echo $event['category'] == 'Product Knowledge' ? 'selected' : ''; ?>>Product Knowledge</option>
                <option value="Promotion" <?php echo $event['category'] == 'Promotion' ? 'selected' : ''; ?>>Promotion</option>
                <option value="Lifestyle" <?php echo $event['category'] == 'Lifestyle' ? 'selected' : ''; ?>>Lifestyle</option>
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="assignee_username" required>
                ...
                <option value="<?php echo htmlspecialchars($user['username']); ?>" 
                        <?php echo $event['assignee_username'] == $user['username'] ? 'selected' : ''; ?>>
        ...
        <div class="form-group">
            ...
            <input type="date" name="brief_date" value="<?php echo $event['brief_date']; ?>" required>
        </div>
        
        <div class="form-group">
            <label>วันที่และเวลาโพสต์ (ถ้ามี)</label>
            <div class="date-time-group">
                <input type="date" name="post_date" value="<?php echo $event['post_date'] ?? ''; ?>" onchange="calculateEngageDate()">
                <input type="time" name="post_time" value="<?php echo $event['post_time'] ?? '12:00'; ?>">
            </div>
        ...
        <div class="form-group">
            ...
            <div class="checkbox-group">
                <label><input type="checkbox" name="platform[]" value="FB" <?php echo in_array('FB', $selected_platforms) ? 'checked' : ''; ?>> Facebook</label>
                <label><input type="checkbox" name="platform[]" value="IG" <?php echo in_array('IG', $selected_platforms) ? 'checked' : ''; ?>> Instagram</label>
                <label><input type="checkbox" name="platform[]" value="TT" <?php echo in_array('TT', $selected_platforms) ? 'checked' : ''; ?>> TikTok</label>
                <label><input type="checkbox" name="platform[]" value="Website" <?php echo in_array('Website', $selected_platforms) ? 'checked' : ''; ?>> Website</label>
                <label><input type="checkbox" name="platform[]" value="Youtube" <?php echo in_array('Youtube', $selected_platforms) ? 'checked' : ''; ?>> Youtube</label>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="status" required>
                <option value="planned" <?php echo $event['status'] == 'planned' ? 'selected' : ''; ?>>วางแผน</option>
                <option value="in_progress" <?php echo $event['status'] == 'in_progress' ? 'selected' : ''; ?>>กำลังทำ</option>
                <option value="posted" <?php echo $event['status'] == 'posted' ? 'selected' : ''; ?>>โพสต์แล้ว</option>
                <option value="completed" <?php echo $event['status'] == 'completed' ? 'selected' : ''; ?>>เสร็จสิ้น</option>
                <option value="cancelled" <?php echo $event['status'] == 'cancelled' ? 'selected' : ''; ?>>ยกเลิก</option>
            </select>
        </div>
        
        <div class="button-group">
            <button type="submit" class="btn btn-primary">💾 บันทึกการแก้ไข</button>
            <a href="calendar_detail.php?id=<?php echo $event_id; ?>" class="btn btn-secondary">❌ ยกเลิก</a>
            <button type="button" class="btn btn-danger" onclick="confirmDelete()">🗑️ ลบงาน</button>
        </div>
    </form>
</div>
...
function confirmDelete() {
    if (confirm('คุณแน่ใจหรือไม่ที่จะลบงานนี้?\n\nการลบจะไม่สามารถกู้คืนได้')) {
        window.location.href = 'calendar_delete.php?id=<?php echo $event_id; ?>';
    }
}
