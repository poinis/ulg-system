...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_engage'])) {
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        try {
            ...
            // ดึงข้อมูลงาน
            $sql_event = "SELECT job_title, category FROM topologie_calendar_events WHERE id = ?";
...
$sql_pending = "SELECT e.*, eng.reach, eng.impressions, eng.likes, eng.comments, eng.shares, eng.saves, eng.note
                FROM topologie_calendar_events e
                LEFT JOIN topologie_calendar_engage eng ON e.id = eng.event_id
