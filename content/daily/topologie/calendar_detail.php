...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_engage'])) {
    ...
    $sql_check = "SELECT id FROM topologie_calendar_engage WHERE event_id = ?";
    ...
    if ($engage_exists) {
        // อัปเดต
        $sql = "UPDATE topologie_calendar_engage 
        ...
        $sql = "INSERT INTO topologie_calendar_engage 
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        $sql_update_status = "UPDATE topologie_calendar_events SET engage_status = 'completed' WHERE id = ?";
        ...
        try {
            ...
            $message .= "\n🔗 ดูรายละเอียด: https://www.weedjai.com/content/daily/calendar_detail.php?id=$event_id";
...
// ดึงข้อมูลงาน
$sql_event = "SELECT * FROM topologie_calendar_events WHERE id = ?";
$stmt_event = mysqli_prepare($conn, $sql_event);
mysqli_stmt_bind_param($stmt_event, "i", $event_id);
mysqli_stmt_execute($stmt_event);
...
$engage_data = null;
$sql_engage = "SELECT * FROM topologie_calendar_engage WHERE event_id = ?";
...
<div class="container">
    ...
    <div class="card">
        <div class="button-group">
            <a href="index.php" class="btn btn-secondary">🔙 กลับปฏิทิน</a>
