...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_engage'])) {
    ...
    $sql_check = "SELECT id FROM hooga_calendar_engage WHERE event_id = ?";
    ...
    if ($engage_exists) {
        // อัปเดต
        $sql = "UPDATE hooga_calendar_engage 
        ...
        $sql = "INSERT INTO hooga_calendar_engage 
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        $sql_update_status = "UPDATE hooga_calendar_events SET engage_status = 'completed' WHERE id = ?";
        ...
        try {
            ...
            // ดึงข้อมูลงาน
            $sql_event = "SELECT job_title, category FROM hooga_calendar_events WHERE id = ?";
...
$sql_pending = "SELECT e.*, eng.reach, eng.impressions, eng.likes, eng.comments, eng.shares, eng.saves, eng.note
                ...
                LEFT JOIN hooga_calendar_engage eng ON e.id = eng.event_id
