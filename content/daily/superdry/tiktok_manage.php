...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_tiktok'])) {
    ...
    if ($exists) {
        // อัปเดต
        $sql = "UPDATE superdry_tiktok_posts SET tiktok_video_id = ?, tiktok_url = ? WHERE event_id = ?";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssi", $tiktok_video_id, $tiktok_url, $event_id);
    } else {
        // สร้างใหม่
        $sql = "INSERT INTO superdry_tiktok_posts (event_id, tiktok_video_id, tiktok_url) VALUES (?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
...
$sql_events = "SELECT e.*, t.tiktok_video_id, t.tiktok_url 
               ...
               LEFT JOIN superdry_tiktok_posts t ON e.id = t.event_id
