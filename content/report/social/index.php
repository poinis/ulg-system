...
try {
    ...
    // Default dates: current period = 7 days (ending 2 days ago), compare = 7 days before
    $now = new DateTime();
    $endDate1Default = (clone $now)->modify('-2 days')->format('Y-m-d');
    $endDate1 = $_GET['end_date1'] ?? $endDate1Default;
    ...
    $postsPrevious = getPosts($pdo, $startDate2, $endDate2, $platformParam, 100);
...
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="nav-links">
                <a href="../index.php" class="nav-link">⬅ กลับหน้าหลัก</a>
                <a href="upload.php" class="nav-link">📤 อัพโหลด</a>
        ...
        <!-- Weekly Change Table -->
        <div class="card">
            <h2 class="card-title">📊 Weekly Change Comparison</h2>
            <div style="overflow-x: auto;">
                <table class="posts-table">
                    <thead>
                        <tr>
                            <th>Platform</th>
                            <th class="number">Posts Change</th>
                            <th class="number">Views Change</th>
                            <th class="number">Likes Change</th>
                            <th class="number">Comments Change</th>
                            <th class="number">Shares Change</th>
                            <th class="number">Saves Change</th>
                            <th class="number">ER Change</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        foreach (['Facebook', 'Instagram', 'TikTok'] as $platform):
                            $current = $currentByPlatform[$platform] ?? ['views' => 0, 'likes' => 0, 'comments' => 0, 'shares' => 0, 'saves' => 0, 'posts' => 0];
                            $previous = $previousByPlatform[$platform] ?? ['views' => 0, 'likes' => 0, 'comments' => 0, 'shares' => 0, 'saves' => 0, 'posts' => 0];
                            
                            $postsChange = calculateChange($current['posts'], $previous['posts']);
                            $viewsChange = calculateChange($current['views'], $previous['views']);
                            $likesChange = calculateChange($current['likes'], $previous['likes']);
                            $commentsChange = calculateChange($current['comments'], $previous['comments']);
                            $sharesChange = calculateChange($current['shares'], $previous['shares']);
                            $savesChange = calculateChange($current['saves'], $previous['saves']);
                            
                            $currentER = calculateER($current['likes'], $current['comments'], $current['saves'], $platforms[$platform]['followers']);
                            $previousER = calculateER($previous['likes'], $previous['comments'], $previous['saves'], $platforms[$platform]['followers']);
                            $erChange = calculateChange($currentER, $previousER);
                        ?>
                        <tr>
                            <td>
                                <span class="badge <?php echo strtolower($platform); ?>">
                                    <?php echo $platform === 'Facebook' ? '📘' : ($platform === 'Instagram' ? '📸' : '🎵'); ?>
                                    <?php echo $platform; ?>
                                </span>
                            </td>
                            <td class="number">
                                <span class="change <?php echo $postsChange >= 0 ? 'positive' : 'negative'; ?>" style="padding: 4px 8px;">
                                    <?php echo $postsChange >= 0 ? '+' : ''; ?><?php echo number_format($postsChange, 2); ?>%
                                </span>
                            </td>
                            <td class="number">
                                <span class="change <?php echo $viewsChange >= 0 ? 'positive' : 'negative'; ?>" style="padding: 4px 8px;">
                                    <?php echo $viewsChange >= 0 ? '+' : ''; ?><?php echo number_format($viewsChange, 2); ?>%
                                </span>
                            </td>
                            <td class="number">
                                <span class="change <?php echo $likesChange >= 0 ? 'positive' : 'negative'; ?>" style="padding: 4px 8px;">
                                    <?php echo $likesChange >= 0 ? '+' : ''; ?><?php echo number_format($likesChange, 2); ?>%
                                </span>
                            </td>
                            <td class="number">
                                <span class="change <?php echo $commentsChange >= 0 ? 'positive' : 'negative'; ?>" style="padding: 4px 8px;">
                                    <?php echo $commentsChange >= 0 ? '+' : ''; ?><?php echo number_format($commentsChange, 2); ?>%
                                </span>
                            </td>
                            <td class="number">
                                <span class="change <?php echo $sharesChange >= 0 ? 'positive' : 'negative'; ?>" style="padding: 4px 8px;">
                                    <?php echo $sharesChange >= 0 ? '+' : ''; ?><?php echo number_format($sharesChange, 2); ?>%
                                </span>
                            </td>
                            <td class="number">
                                <span class="change <?php echo $savesChange >= 0 ? 'positive' : 'negative'; ?>" style="padding: 4px 8px;">
                                    <?php echo $savesChange >= 0 ? '+' : ''; ?><?php echo number_format($savesChange, 2); ?>%
                                </span>
                            </td>
                            <td class="number">
                                <span class="change <?php echo $erChange >= 0 ? 'positive' : 'negative'; ?>" style="padding: 4px 8px;">
                                    <?php echo $erChange >= 0 ? '+' : ''; ?><?php echo number_format($erChange, 2); ?>%
                                </span>
                            </td>
                        </tr>
                        <?php endforeach; ?>
        ...
        <div class="card posts-section">
            <h2 class="card-title">📝 Posts ช่วงเวลาปัจจุบัน (<?php echo count($posts); ?> posts)</h2>
            <div class="period-label current" style="margin-bottom: 15px;">
                📅 <?php echo date('d/m/Y', strtotime($startDate1)); ?> - <?php echo date('d/m/Y', strtotime($endDate1)); ?>
            </div>
            <div class="table-scroll">
                <table class="posts-table">
                    ...
                    <tfoot style="background: rgba(255,255,255,0.05); font-weight: bold;">
                        <tr>
                            <td colspan="4" style="text-align: right; padding-right: 20px;">📊 ค่าเฉลี่ย:</td>
                            <td class="number"><?php echo count($posts) > 0 ? number_format(array_sum(array_column($posts, 'views')) / count($posts)) : '0'; ?></td>
                            <td class="number"><?php echo count($posts) > 0 ? number_format(array_sum(array_column($posts, 'likes')) / count($posts)) : '0'; ?></td>
                            <td class="number"><?php echo count($posts) > 0 ? number_format(array_sum(array_column($posts, 'comments')) / count($posts)) : '0'; ?></td>
                            <td class="number"><?php echo count($posts) > 0 ? number_format(array_sum(array_column($posts, 'shares')) / count($posts)) : '0'; ?></td>
                            <td class="number er-cell">
                                <?php 
                                if (count($posts) > 0) {
                                    $totalER = 0;
                                    foreach ($posts as $p) {
                                        $pFollowers = $platforms[$p['social']]['followers'] ?? 10000;
                                        $pSaves = $p['saves'] + $p['favorites'];
                                        $totalER += calculateER($p['likes'], $p['comments'], $pSaves, $pFollowers);
                                    }
                                    echo number_format($totalER / count($posts), 2);
                                } else {
                                    echo '0.00';
                                }
                                ?>%
        ...
        <!-- Posts Table - Previous Period -->
        <div class="card posts-section">
            <h2 class="card-title">📝 Posts ช่วงเวลาเปรียบเทียบ (<?php echo count($postsPrevious); ?> posts)</h2>
            <div class="period-label previous" style="margin-bottom: 15px;">
                📅 <?php echo date('d/m/Y', strtotime($startDate2)); ?> - <?php echo date('d/m/Y', strtotime($endDate2)); ?>
            </div>
            <div class="table-scroll">
                <table class="posts-table">
                    <thead>
                    ...
                    <tbody>
                        <?php foreach ($postsPrevious as $post): 
                            $postFollowers = $platforms[$post['social']]['followers'] ?? 10000;
                            $postSaves = $post['saves'] + $post['favorites'];
                            $postER = calculateER($post['likes'], $post['comments'], $postSaves, $postFollowers);
                        ?>
...
<tfoot style="background: rgba(255,255,255,0.05); font-weight: bold;">
                        <tr>
                            <td colspan="4" style="text-align: right; padding-right: 20px;">📊 ค่าเฉลี่ย:</td>
                            <td class="number"><?php echo count($postsPrevious) > 0 ? number_format(array_sum(array_column($postsPrevious, 'views')) / count($postsPrevious)) : '0'; ?></td>
                            <td class="number"><?php echo count($postsPrevious) > 0 ? number_format(array_sum(array_column($postsPrevious, 'likes')) / count($postsPrevious)) : '0'; ?></td>
                            <td class="number"><?php echo count($postsPrevious) > 0 ? number_format(array_sum(array_column($postsPrevious, 'comments')) / count($postsPrevious)) : '0'; ?></td>
                            <td class="number"><?php echo count($postsPrevious) > 0 ? number_format(array_sum(array_column($postsPrevious, 'shares')) / count($postsPrevious)) : '0'; ?></td>
                            <td class="number er-cell">
                                <?php 
                                if (count($postsPrevious) > 0) {
                                    $totalER = 0;
                                    foreach ($postsPrevious as $p) {
                                        $pFollowers = $platforms[$p['social']]['followers'] ?? 10000;
                                        $pSaves = $p['saves'] + $p['favorites'];
                                        $totalER += calculateER($p['likes'], $p['comments'], $pSaves, $pFollowers);
                                    }
                                    echo number_format($totalER / count($postsPrevious), 2);
                                } else {
                                    echo '0.00';
                                }
                                ?>%
