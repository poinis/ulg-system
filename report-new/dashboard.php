...
// ✨ 1. ดึงข้อมูลร้านค้าทั้งหมด (Active Only) ไม่ต้องทำ Map แล้ว
$all_stores = getActiveStores($db);

// ✨ 2. ดึงยอดขายรายวัน (GROUP BY store_code ตรงๆ)
...
$daily_data = $daily_stmt->fetchAll(PDO::FETCH_UNIQUE); // ใช้ store_code เป็น Key

// ✨ 3. ดึงยอดขายรายเดือน (GROUP BY store_code ตรงๆ)
$monthly_stmt = $db->prepare("
    SELECT 
        store_code,
        SUM(tax_incl_total) as monthly_sales
    FROM daily_sales
    WHERE sale_date BETWEEN ? AND ?
    GROUP BY store_code
");
$monthly_stmt->execute([$date_from, $date_to]);
$monthly_data = $monthly_stmt->fetchAll(PDO::FETCH_UNIQUE);

// ✨ 4. ดึงเป้าหมาย (GROUP BY store_code ตรงๆ)
...
$targets = $targets_stmt->fetchAll(PDO::FETCH_UNIQUE);
...
foreach ($all_stores as $store) {
    ...
    // ดึงข้อมูลจาก Array ที่เตรียมไว้โดยใช้ Code ตรงๆ
    $daily_sale = $daily_data[$code]['daily_sales'] ?? 0;
    $monthly_sale = $monthly_data[$code]['monthly_sales'] ?? 0;
    $daily_target = $targets[$code]['daily_target'] ?? 0;
    $monthly_target = $targets[$code]['monthly_target'] ?? 0;
    $trans_count = $daily_data[$code]['transaction_count'] ?? 0;

    // ✨ เงื่อนไข: ซ่อนสาขาที่มียอดขายช่วงเดือนเป็น 0 (ถ้าต้องการ)
    // if ($monthly_sale <= 0) continue; 
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { background: rgba(2, 136, 209, 0.95); backdrop-filter: blur(15px); padding: 25px; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.1); border-bottom: 1px solid rgba(255, 255, 255, 0.2); }
        ...
        .date-input-group label { color: white; font-size: 13px; font-weight: 700; text-shadow: 1px 1px 3px rgba(0, 0, 0, 0.2); }
        .date-input-group input { padding: 12px 16px; border: 2px solid rgba(255, 255, 255, 0.3); border-radius: 10px; background: rgba(255, 255, 255, 0.95); font-size: 14px; font-family: 'Sarabun', sans-serif; font-weight: 600; transition: all 0.3s ease; }
        .date-input-group input:focus { outline: none; border-color: white; background: white; box-shadow: 0 0 0 4px rgba(255, 255, 255, 0.3); }
        .btn-search { padding: 12px 28px; background: white; color: #667eea; border: none; border-radius: 10px; cursor: pointer; font-weight: 700; font-size: 15px; transition: all 0.3s ease; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.2); display: flex; align-items: center; gap: 8px; }
        .btn-search:hover { background: #f0f0f0; transform: translateY(-3px); box-shadow: 0 6px 20px rgba(0, 0, 0, 0.3); }
        ...
        .nav-btn { display: inline-flex; align-items: center; gap: 10px; padding: 12px 24px; background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); color: #333; text-decoration: none; border-radius: 12px; font-weight: 600; font-size: 14px; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); border: 2px solid transparent; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1); }
        .nav-btn:hover { background: linear-gradient(135deg, #0288d1 0%, #0097a7 100%); color: white; transform: translateY(-3px); box-shadow: 0 6px 20px rgba(2, 136, 209, 0.4); }
        ...
        .stat-card { background: white; padding: 30px; border-radius: 20px; box-shadow: 0 8px 30px rgba(0, 0, 0, 0.15); transition: all 0.3s ease; position: relative; overflow: hidden; }
        .stat-card::before { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 4px; background: linear-gradient(90deg, #0288d1 0%, #0097a7 100%); }
        ...
        .stat-value { font-size: 36px; font-weight: 800; background: linear-gradient(135deg, #0288d1 0%, #0097a7 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; margin-bottom: 8px; }
        ...
        th { background: linear-gradient(135deg, #00588bff 0%, #00588bff 100%); color: white; font-weight: 700; font-size: 14px; text-transform: uppercase; letter-spacing: 0.5px; }
        th:first-child { border-radius: 10px 0 0 0; }
        th:last-child { border-radius: 0 10px 0 0; }
        td { font-size: 14px; }
        .number { text-align: right; font-family: 'Courier New', monospace; font-weight: 600; }
        tbody tr { transition: all 0.3s ease; }
        tbody tr:hover { background: linear-gradient(135deg, #e1f5fe 0%, #ffffff 100%); transform: scale(1.01); }
        .total-row { background: linear-gradient(135deg, #8fd7fdff 0%, #00a8bbff 100%); color: black; font-weight: 800; }
        ...
        .progress-fill { height: 100%; background: linear-gradient(90deg, #0288d1, #0097a7); transition: width 0.6s ease; position: relative; overflow: hidden; }
        ...
        @media (max-width: 768px) { .header-content { flex-direction: column; align-items: flex-start; } .header-title { font-size: 24px; } .date-controls { width: 100%; } .date-input-group { flex: 1; min-width: 120px; } .stats-grid { grid-template-columns: 1fr; } .table-card { padding: 20px; } th, td { padding: 12px 8px; font-size: 12px; } }
        @keyframes fadeIn { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
        .stat-card, .table-card { animation: fadeIn 0.6s ease-out; }
